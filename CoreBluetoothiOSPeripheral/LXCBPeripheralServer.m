...
- (id)initWithDelegate:(id<LXCBPeripheralServerDelegate>)delegate {
  self = [super init];
    delegate = [[LXCBAppDelegate alloc]init];
...
- (void)sendToSubscribers:(NSData *)data {
  if (self.peripheral.state != CBPeripheralManagerStatePoweredOn) {
    ...
    weight = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
    [delegate saveToUserDefaults:[NSString stringWithFormat:@"%@ lbs",weight]];
    [delegate insertData];
